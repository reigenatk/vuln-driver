...
namespace pe_mapper
{
	...
	public:
		...
		inline auto read_image(const std::string_view image_path, const char* entrypoint_export_name) -> bool
		{
			...
			// loads the entire image into the vector of bytes
			...
			// get the rva of exported entry point and set it
...
#undef max
		...
		// so given any mapped export, it gets the offset then returns that offset + the mapped file ptr?
		...
		__forceinline auto resolve_imports() const -> bool
		{
			...
			if (import_dir.VirtualAddress && import_dir.Size) // only if we have any imports to resolve
			{
				...
				for (auto import_desc = this->translate<PIMAGE_IMPORT_DESCRIPTOR>(import_dir.VirtualAddress); import_desc->FirstThunk; import_desc++)
				{
					...
					// go through all of the modules. First Thunk in import descriptor points to address that contains all the imports, so here thunk_data.AddressOfData
					for (auto thunk_data = this->translate<PIMAGE_THUNK_DATA>(import_desc->FirstThunk); thunk_data->u1.AddressOfData; thunk_data++)
					{
						// this is the first address of first import
...
#define IMR_RELOFFSET(x)			(x & 0xFFF)
			...
			// do a switch on which type of RELOC it is (first 4 bits)
			switch (data >> 12 & 0xF)
			{
			...
			{
				// just add to the value of the address inside memory, the image base difference
...
#undef IMR_RELOFFSET
		...
		__forceinline auto resolve_relocations() -> bool
		{
			...
			if (reloc_dir.VirtualAddress && reloc_dir.Size) // only if we have any relocations to resolve
			{
				...
				{
					// found how many entries to reloc in this block
		...
		// finds which section a virtual address is in, and returns corresponding pointer to local image file
